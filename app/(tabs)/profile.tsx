...
export default function Profile() {
  ...
  //filling the details using redux
  ...
  const saveProfile = async () => {
    if (isSaving) return;
    setIsSaving(true); //waiting stage
    ...
    try {
      ...
      if (sessionUser.id) {
        await updateRecord(TABLES.USERS, updatedUser, "id = ?", [
          sessionUser.id,
        ]);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white p-4">
      ...
      >
        ...
        <Text className="text-center text-xl font-[Futura] font-bold m-3">
          Manage your profile
        </Text>
        ...
        <View className="space-y-4">
          ...
          <View
            className={`flex-row items-center rounded border ${
          ...
          >
            <Text className="text-red-500 font-[FuturaMedium]">Log out</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
