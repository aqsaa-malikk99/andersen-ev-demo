...
export default function AuthCard({
  ...
  return (
    <View className="w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
      ...
      <Text className="text-left font-[Futura] text-1xl text-green mb-3 ">
        {title}
      </Text>
      {subtitle && (
      ...
      {fields.map((field, i) => (
        <View key={i} className="mb-4">
          <Text className="font-semibold mb-1 font-[Futura]">
            {field.label}
          </Text>
          ...
          {field.type === "date" ? (
            <>
              ...
              >
                <Text  className={`font-[Futura] ${
                    dob ? "text-black" : "text-gray-500"
                }`}>
              ...
              <DateTimePickerModal
                isVisible={showDOB}
                mode="date"
                maximumDate={new Date(new Date().setFullYear(new Date().getFullYear() - 16))} // at least 16 years old
                onConfirm={(date) => {
                    console.log(date);
                  if (date) {
      ...
      >
        <Text className="text-white text-center font-[Futura] font-semibold">
          {primaryButtonText}
        </Text>
      </TouchableOpacity>
      ...
      <View className="flex-row items-center my-4">
        ...
        <Text className="mx-2 text-gray-500 text-sm font-[Futura]">
          {footerText}
        </Text>
        <View className="flex-1 h-px bg-gray-300" />
      </View>
      ...
      <TouchableOpacity
        className="bg-green py-3 rounded-md"
