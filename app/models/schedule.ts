import { createRecord, readRecords, updateRecord, deleteRecord } from "@/app/db/crud";
...
export async function getSchedules(userId: string): Promise<Schedule[]> {
    const records = await readRecords(TABLES.SCHEDULES, "userId = ?", [userId]) as ScheduleData[];
    return records.map(rec => new Schedule({
        ...rec,
...
export async function getScheduleById(id: string): Promise<Schedule | null> {
    const records = await readRecords(TABLES.SCHEDULES, "id = ?", [id]) as ScheduleData[];
